...
--
-- 맵 레지스트리(멀티맵): 팰월드 1.0 은 본섬(MainMap) 사각형 밖에 별도 맵을 추가했다(월드트리='Tree').
-- 각 맵은 자기 (경계 사각형 + 전용 지도 텍스처)를 가진다. 플레이어가 어느 맵에 서 있는지는 좌표를
-- 경계에 넣어 판정(mapAt)하고, 그 맵의 (텍스처 + 경계 + 줌)으로 통째 스왑한다.
...
-- → 호출부가 그동안 미니맵을 숨기고 다음 틱에 재시도한다. 매 틱 6000+ Texture2D 재스캔 방지 = 캐시.
...
local function build(world)
    ...
    local bmap = MAPS[1]   -- 초기화용(본섬). 실제 활성 맵/텍스처는 루프가 매 틱 판정해 스왑한다.
...
LoopAsync(33, function()   -- ~30fps: 지도 패닝/마커 부드럽게(버벅임 해소)
    pcall(function()
        ExecuteInGameThread(function()
            ...
            -- 플레이어가 선 맵 판정 + 그 맵 전용 텍스처 확보(멀티맵). 어느 맵에도 안 들면 am=nil.
            local am = mapAt(x, y)
