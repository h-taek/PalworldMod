...
--
-- 맵 경계: Min=(-999940,-738920) Max=(447900,708920), 스팬 1,447,840.
--   변환(SWAP+세로뒤집기): u(가로)=(worldY-MinY)/스팬,  v(세로)=1-(worldX-MinX)/스팬.

local UEHelpers = require("UEHelpers")

local MIN_X, MIN_Y = -999940.0, -738920.0
local MAX_X, MAX_Y =  447900.0,  708920.0
...
local function readPlayer()
    ...
    local x, y; pcall(function() x = loc.X; y = loc.Y end)
...
LoopAsync(33, function()   -- ~30fps: 지도 패닝/마커 부드럽게(버벅임 해소)
    pcall(function()
        ExecuteInGameThread(function()
            ...
            -- 메뉴 열림 감지 → 미니맵 통째 숨김. Palworld 메뉴 UI는 우리 뷰포트 위젯보다 상위
            --   레이어라 ZOrder로는 뒤로 못 보낸다 → 메뉴 뜨면(마우스 커서 표시) 숨겨서 안 가리게 한다.
            if isv(gWidget) then
                local pc = UEHelpers.GetPlayerController()
                local menuOpen = false
                pcall(function() menuOpen = (pc and pc.bShowMouseCursor == true) end)
                pcall(function() gWidget:SetVisibility(menuOpen and 1 or 0) end)  -- 1=Collapsed, 0=Visible
            end
